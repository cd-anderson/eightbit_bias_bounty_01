...
# Modified 11/2022 for EightBitBiasBounty
...
from semilearn.core.utils import get_logger, EMA
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix

from utils import disparity_score, dump_to_json, eightbit_score


class EightBitBiasBountyTrainer:
    ...
    def fit(self, train_lb_loader, train_ulb_loader, eval_loader):
        ...
        best_eval_acc = 0.0
        best_epoch = 0
        best_disparity_epoch = 0
        best_eval_disparity = 9999999
        best_eval_score = 0
        best_score_epoch = 0
        ...
        for epoch in range(self.config.epoch):
            ...
            if result['acc'] > best_eval_acc:
                ...
                best_epoch = epoch
                self.logger.info("Best acc {:.4f} saving model...".format(best_eval_acc))
                self.algorithm.save_model('model_best_acc.pth', self.save_path)
                dump_to_json(result, 'model_best_acc.json', self.save_path)

            if result['disparity'] < best_eval_disparity:
                best_eval_disparity = result['disparity']
                best_disparity_epoch = epoch
                self.logger.info("Best disparity {:.4f} saving model...".format(best_eval_disparity))
                self.algorithm.save_model('model_best_disparity.pth', self.save_path)
                dump_to_json(result, 'model_best_disparity.json', self.save_path)

            if result['eightbit_score'] > best_eval_score:
                best_eval_score = result['eightbit_score']
                best_score_epoch = epoch
                self.logger.info("Best eightbit {:.4f} saving model...".format(best_eval_score))
                self.algorithm.save_model('model_best_eightbit.pth', self.save_path)
                dump_to_json(result, 'model_best_eightbit.json', self.save_path)

        self.logger.info("Best acc {:.4f} at epoch {:d}".format(best_eval_acc, best_epoch))
        self.logger.info("Best disparity {:.4f} at epoch {:d}".format(best_eval_disparity, best_disparity_epoch))
        self.logger.info("Best eightbit_score {:.4f} at epoch {:d}".format(best_eval_score, best_score_epoch))
    ...
    def evaluate(self, data_loader, use_ema_model=False):
        ...
        disparity = disparity_score(cf_mat)
        self.logger.info("confusion matrix")
        self.logger.info(cf_mat)
        result_dict = {'acc': top1, 'precision': precision, 'recall': recall, 'f1': f1, 'disparity': disparity}
        result_dict['eightbit_score'] = eightbit_score(result_dict)
